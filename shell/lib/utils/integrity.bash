...
function get_source_latest(){
  ...
  fail_if "not able to get latest source: $1"
}
...
function get_file_time(){
  ...
  fail_if "not able to get file time: $1"
}
...
function assert_set_env_is_up_to_date(){
  ...
  export current_bash_source_latest="$(get_source_latest "${PEMBA_PATHS_SHELL_HOME}")"
  ...
  export current_bash_source_latest="$current_bash_source_latest$(get_file_time "${PATHS_PROJECT_HOME}/.pemba")"
...
######################################################    FAILURE    ##############################################
    ...
    bash --init-file ${PATHS_PROJECT_HOME}/.pemba
###################################################################################################################
"
  fi
}
